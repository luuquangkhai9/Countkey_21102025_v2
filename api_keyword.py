...
# Sử dụng RotatingFileHandler để giới hạn kích thước file log
file_handler = RotatingFileHandler(
    "api.log",
    maxBytes=10*1024*1024,  # Giới hạn 10MB
    backupCount=5,          # Giữ lại tối đa 5 file cũ (api.log.1, api.log.2, ...)
    encoding='utf-8'
)
...
DISABLE_CACHE = False  # Set True để tắt hoàn toàn cache khi develop/test
...
@app.post("/search_keywords/")
async def search_keywords_api(request: SearchRequest, response: Response, bypass_cache: bool = False):
    # Tắt hoàn toàn cache nếu DISABLE_CACHE = True
    if DISABLE_CACHE:
        bypass_cache = True
...
async def search_keywords(request: SearchRequest, response: Response):
    try:
        ...
        # Sử dụng index khác nhau cho popular và trend
        # key-osint1-current: chứa keywords_top, hashtags_top (popular)
        # key-trend-v2-000001: chứa keywords_trend với cấu trúc mới (trend)
        index_name = "key-osint1-current" if request.type_top == "popular" else "key-trend-current-v2"
        if (end_date - start_date).days > 80:
            ...
            # Xây dựng query khác nhau cho popular và trend
            if request.type_top == "popular":
                # Index key-osint1-current sử dụng date format MM_DD_YYYY
                query = {
                    "bool": {
                        "must": [
                ...
                # Index key-trend-v2-000001 sử dụng date format MM/DD/YYYY
                # và có thể filter theo topic_id (bao gồm "all")
                topic_ids_for_query = request.topic_ids if request.topic_ids else ["all"]
                query = {
                    "bool": {
                        "must": [
                            {"terms": {"type": request.type}},
                            {"terms": {"topic_id": topic_ids_for_query}},
                            {"terms": {"tenant_id": request.tenancy_ids}},
                            {
                                "range": {
                                    "date": {
                                        "gte": start_date.strftime("%m/%d/%Y"),
                                        "lte": end_date.strftime("%m/%d/%Y")
                                    }
                                }
                            }
                        ]
            ...
            # Debug log query
            api_logger.info(f"ES Query: {query}")
            api_logger.info(f"Index: {index_name}")

            start_query_time = time.time()

            es_response = es_db.search(index=index_name, body={"query": query, "size": 10000},  timeout="60s" )
            api_logger.info(f"ES Response hits count: {len(es_response['hits']['hits'])}")
            ...
            if request.type_top == "popular":
                ...
                for doc_source in hits:
                    ...
                    keyword_aggregate = defaultdict(lambda: {"record": 0, "top_20_days": 0, "trends": 0, "score": 0})
                    for hit in hits:
                        hit_type = hit['_source'].get('type', '')
                        # Xử lý trường hợp type là list
                        if isinstance(hit_type, list):
                            hit_type = hit_type[0] if hit_type else ''
                        ...
                        # Chỉ lấy top 50 keywords có isTrend = True
                        trending_keywords = [kw for kw in keywords_list if kw.get("isTrend", False)][:50]
                        
                        for i, keyword_data in enumerate(trending_keywords):
                            keyword = keyword_data.get("keyword")
                            if not keyword:
                                continue

                            keyword_aggregate[keyword]["record"] += keyword_data.get("record", 0)
                            # Lấy score cao nhất cho keyword
                            current_score = keyword_data.get("score", 0)
                            if current_score > keyword_aggregate[keyword]["score"]:
                                keyword_aggregate[keyword]["score"] = current_score

                            if hit_type in {"voz", "otofun", "xamvn", "reddit"}:
                                continue  # forum: không tính top/trend

                            if hit_type in {"facebook", "media", "youtube"}:
                                if i < 10:
                    ...
                    # Chuyển đổi _ thành space
                    sorted_keywords = [(k.replace('_', ' '), v) for k, v in sorted_keywords]
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("api_keyword:app", host="0.0.0.0", port=55555, reload=True)
